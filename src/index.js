...
import { handleCommercialAdmin } from "./admin-commercial.js";
import { handleWalletAdmin } from "./admin-wallet.js";
import { adminConsoleHtml } from "./admin-ui.js";
import { chargeWallet, getWallet } from "./wallet.js";
...
async function authenticate(request,env){
  ...
  const auth=request.headers.get("authorization")||"";if(!auth.startsWith("Bearer "))return{ok:false,response:json({error:"missing_api_key"},401)};
  const rawKey=auth.slice(7).trim();if(!rawKey.startsWith("agw_"))return{ok:false,response:json({error:"invalid_api_key"},401)};
  const keyHash=await sha256Hex(rawKey);
  const row=await env.DB.prepare(`SELECT k.id AS api_key_id,k.tenant_id,k.status,t.name AS tenant_name,t.status AS tenant_status,t.billing_mode FROM api_keys k JOIN tenants t ON t.id=k.tenant_id WHERE k.key_hash=?1 LIMIT 1`).bind(keyHash).first();
...
async function prepaidPreflight(env,principal){
  if(principal.billing_mode!=="prepaid")return{ok:true,wallet:null};
  const wallet=await getWallet(env,principal.tenant_id);const minimum=Math.max(0,Number(env.PREPAID_MIN_BALANCE_TWD||1));
  if(wallet?.status!=="active")return{ok:false,response:json({error:"wallet_not_active"},402)};
  if(Number(wallet?.balance_twd||0)<minimum)return{ok:false,response:json({error:"insufficient_wallet_balance",balance_twd:Number(wallet?.balance_twd||0),minimum_required_twd:minimum},402)};
  return{ok:true,wallet};
}

async function handleRun(request,env,ctx){
  const id=requestId(request);const auth=await authenticate(request,env);if(!auth.ok)return auth.response;
  const walletCheck=await prepaidPreflight(env,auth.principal);if(!walletCheck.ok)return walletCheck.response;
  const parsed=await parseJson(request);if(!parsed.ok)return parsed.response;
  ...
  const attempts=[];
  for(const candidate of route){const attemptStarted=Date.now();try{
    const providerResult=await runProvider({env,region,task,input,...candidate});let result=providerResult.result;
    if(candidate.operation==="image.edit"&&Array.isArray(providerResult.result?.images)){const stored=await storeGeneratedImages(env,{tenantId:auth.principal.tenant_id,requestId:id,provider:candidate.provider,images:providerResult.result.images});if(stored?.length)result={images:stored.map(asset=>({asset_key:asset.key,content_type:asset.content_type,size:asset.size,download_url:assetDownloadUrl(request,asset.key)})),storage:"r2"}}
    const cost=await calculateProviderCost(env,{provider:candidate.provider,model:candidate.model,region,usage:providerResult.usage});
    const billableAmountTwd=await calculateBillable(env,{tenantId:auth.principal.tenant_id,task,providerCostTwd:cost.twd});
    let wallet=null;
    if(auth.principal.billing_mode==="prepaid"){
      try{wallet=await chargeWallet(env,{tenantId:auth.principal.tenant_id,amountTwd:billableAmountTwd,requestId:id,note:task})}
      catch(error){const latencyMs=Date.now()-startedAt;await updateUsage(env,eventId,{provider:candidate.provider,model:candidate.model,status:"payment_failed",usage:providerResult.usage,cost,billableAmountTwd,latencyMs,errorCode:error?.code||"wallet_charge_failed"});return json({error:error?.code||"wallet_charge_failed",request_id:id,balance_twd:error?.balance_twd??null,required_twd:error?.required_twd??billableAmountTwd},402)}
    }
    const latencyMs=Date.now()-startedAt;await updateUsage(env,eventId,{provider:candidate.provider,model:candidate.model,status:"success",usage:providerResult.usage,cost,billableAmountTwd,latencyMs});
    return json({request_id:id,status:"success",task,region,provider:candidate.provider,model:candidate.model,provider_request_id:providerResult.providerRequestId||null,result,usage:{input_units:providerResult.usage?.inputUnits||0,output_units:providerResult.usage?.outputUnits||0,image_count:providerResult.usage?.imageCount||0,provider_cost:cost.amount,provider_currency:cost.currency,provider_cost_twd:Number(cost.twd.toFixed(6)),billable_amount_twd:Number(billableAmountTwd.toFixed(6)),priced:cost.priced},wallet:wallet?{charged_twd:wallet.charged,balance_twd:wallet.balance_twd}:null,attempts});
...
export default{async fetch(request,env,ctx){try{const url=new URL(request.url);
  ...
  if(url.pathname.startsWith("/admin/")){const wallet=await handleWalletAdmin(request,env,url);if(wallet)return wallet;const commercial=await handleCommercialAdmin(request,env,url);if(commercial)return commercial;return await handleAdmin(request,env,url)}
  if(request.method==="GET"&&url.pathname==="/health")return json({ok:true,service:"ai-gateway",version:"0.3.3",environment:env.ENVIRONMENT||"unknown",d1_configured:Boolean(env.DB),r2_configured:Boolean(env.ASSETS),openai_configured:Boolean(env.OPENAI_API_KEY),qwen_cn_configured:Boolean(env.QWEN_CN_API_KEY&&env.QWEN_CN_WORKSPACE_ID),qwen_intl_configured:Boolean(env.QWEN_INTL_API_KEY&&env.QWEN_INTL_WORKSPACE_ID),admin_configured:Boolean(env.ADMIN_API_KEY),timestamp:new Date().toISOString()});
  ...
  if(request.method==="GET"&&url.pathname==="/")return json({service:"AI Gateway",status:"online",version:"0.3.3",console:"/console",endpoints:["GET /health","POST /v1/run","GET /v1/assets/:key","GET /admin/dashboard","POST /admin/tenants","POST /admin/tenants/:id/keys","GET /admin/plans","PUT /admin/tenants/:id/subscription","PUT /admin/tenants/:id/rate-cards","GET /admin/tenants/:id/wallet","POST /admin/tenants/:id/wallet/topup","GET /admin/invoices"]});
